 ...
 *
 * GD32VW55x BLE 5.2 through the vendor's own BLE host (Plan B).
 *
...
extern void *sys_task_create(void *static_tcb, const uint8_t *name, uint32_t *stack_base,
...
extern int32_t sys_queue_init(os_queue_t *queue, int32_t queue_size, uint32_t item_size);
extern void sys_queue_free(os_queue_t *queue);
extern int sys_queue_write(os_queue_t *queue, void *msg, int timeout, bool isr);
extern int sys_queue_read(os_queue_t *queue, void *msg, int timeout, bool isr);
...
extern void ble_coex_evt_notify_register(void (*cb)(uint32_t, uint32_t, uint32_t));
extern void coex_ble_event_notify(uint32_t evt_start, uint32_t evt_window, uint32_t iso_evt);
...
#define GDBLE_ADV_NAME   CONFIG_WIFI_GDWIFI_BLE_VENDOR_NAME
...
static void gdble_adv_evt_handler(ble_adv_evt_t adv_evt, void *p_data, void *p_context)
{
	...
	LOG_DBG("adv state -> %u (set %u)", chg->state, chg->adv_idx);
...
/* Connection events.  Two things have to happen here:
 *
 *  - a connection stops the legacy advertising set and the stack leaves it
 *    stopped, so restart it on every disconnection or the device is never
 *    discoverable again until a reset;
 *  - the GAP service is implemented by the stack but its values come from
 *    the application through *_GET_IND events, each of which must be
 *    answered with the matching *_cfm().  A central reading the device
 *    name during service discovery otherwise waits for its own timeout.
 */

static void gdble_conn_evt_handler(ble_conn_evt_t event, ble_conn_data_u *p_data)
{
	LOG_DBG("conn evt %u (state %u)", event,
		(event == BLE_CONN_EVT_STATE_CHG) ? p_data->conn_state.state : 0xffU);

	switch (event) {
	case BLE_CONN_EVT_STATE_CHG:
		if (p_data->conn_state.state == BLE_CONN_STATE_DISCONNECTD && g_adv_idx != 0xff) {
			LOG_INF("BLE disconnected (reason 0x%x), restarting "
				"advertising",
	...
	case BLE_CONN_EVT_NAME_GET_IND: {
		/* The peer is reading the GAP Device Name characteristic and
		 * the controller waits for our answer: leaving it
		 * unconfirmed stalls the central until its own timeout.
		 */

		uint16_t offset = p_data->name_get_ind.name_offset;
		uint16_t full_len = sizeof(GDBLE_ADV_NAME) - 1;
		uint16_t len = (offset < full_len) ? (full_len - offset) : 0;

		if (len > p_data->name_get_ind.max_name_length) {
			len = p_data->name_get_ind.max_name_length;
		}

		ble_conn_name_get_cfm(p_data->name_get_ind.conn_idx, 0, p_data->name_get_ind.token,
				      full_len, (uint8_t *)GDBLE_ADV_NAME + offset, len);
		break;
	}

	case BLE_CONN_EVT_APPEARANCE_GET_IND:
		/* No appearance is advertised: report "unknown" (0). */
	...
	case BLE_CONN_EVT_SLAVE_PREFER_PARAM_GET_IND: {
		ble_gap_slave_prefer_param_t param = {
			.conn_intv_min = 8,  /* 10 ms  */
			.conn_intv_max = 10, /* 12.5 ms */
			.latency = 0,
			.conn_tout = 200, /* 2 s */
		};

		ble_conn_slave_prefer_param_get_cfm(p_data->slave_prefer_param_get_ind.conn_idx, 0,
						    p_data->slave_prefer_param_get_ind.token,
						    &param);
		break;
	}
...
static void gdble_adv_create(void)
{
	...
	g_gdble_dbg_create_st = ble_adv_create(&adv_param, gdble_adv_evt_handler, NULL);
}
