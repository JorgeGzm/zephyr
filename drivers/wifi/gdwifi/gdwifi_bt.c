...
static void gdble_handle_event(uint8_t *p_header, uint16_t payload_length)
{
	...
	bool discardable = p_header[0] == BT_HCI_EVT_LE_META_EVENT  ? false
			   ...
			   : p_header[0] == BT_HCI_EVT_CMD_STATUS   ? false
								    : false;
	...
	if (payload_length > 0) {
		if (!virtual_hci_get_payload(net_buf_add(buf, payload_length), payload_length)) {
...
static int gdble_send(const struct device *dev, struct net_buf *buf)
{
	...
	case HCI_CMD_MSG_TYPE: {
		...
		if (!virtual_hci_send_command(opcode, len, len > 0 ? &pkt[3] : NULL)) {
			err = -ENOBUFS;
		}
		break;
	}
	case HCI_ACL_MSG_TYPE: {
		...
		if (!virtual_hci_send_acl_data(hdl_flags, len, len > 0 ? &pkt[4] : NULL)) {
			err = -ENOBUFS;
		}
		break;
	}
	case HCI_ISO_MSG_TYPE: {
		...
		if (!virtual_hci_send_iso_data(hdl_flags, len, len > 0 ? &pkt[4] : NULL)) {
			err = -ENOBUFS;
		}
		break;
	}
...
#define GDBLE_DEVICE_INIT(inst)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(inst, NULL, NULL, &gdble_hci_data_##inst, &gdble_config_##inst,      \
			      POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &gdble_drv)
