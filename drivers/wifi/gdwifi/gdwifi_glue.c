...
static const struct device *const gdwifi_console = DEVICE_DT_GET_OR_NULL(DT_CHOSEN(zephyr_console));
...
{
	irq_connect_dynamic(WIFI_PROT_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_PROT_IRQHandler, 0);
	irq_connect_dynamic(WIFI_INTGEN_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_INTGEN_IRQHandler,
			    0);
	irq_connect_dynamic(WIFI_TX_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_TX_IRQHandler, 0);
	irq_connect_dynamic(WIFI_RX_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_RX_IRQHandler, 0);
	irq_connect_dynamic(LA_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, LA_IRQHandler, 0);
	irq_connect_dynamic(WIFI_WKUP_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_WKUP_IRQHandler, 0);
	irq_connect_dynamic(WIFI_INT_IRQn, GDWIFI_IRQ_LEVEL, gdwifi_isr, WIFI_INT_IRQHandler, 0);
}
...
{
	if (line == 2411 && file != NULL && strstr(file, "mm_task") != NULL) {
...
#define GDWIFI_STEP(name, call)                                                                    \
	...
	/* rom_init() gives the ROM copy of mbedTLS its allocator; the copy
	 * built into this driver needs the same (the SDK configuration has no
	 * standard functions).
	 */
	GDWIFI_STEP("mbedtls_platform", mbedtls_platform_set_calloc_free(sys_calloc, sys_mfree));
