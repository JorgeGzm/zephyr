...
/* lwipopts.h - Zephyr shim over the GD32VW55x SDK lwIP options.
 *
 ...
 * reconciles clashes with the Zephyr libc headers.
 */
...
/* The C library <limits.h> defines IOV_MAX as INT_MAX which trips the lwIP
 * sockets.h coherence check; clamp it to what lwIP supports.
 */
...
/* The C library <sys/types.h> (pulled in by the libc headers) already defines
 ...
 * lwIP-owned in this build - the libc <sys/socket.h> must NOT be included
...
/* Keep lwIP from typedefing sa_family_t (the C library already has it); the
