...
/* wifi_netif.h - Zephyr port shim.
 *
 * Shadows the header of the SDK lwIP port.  Here the IP stack is the Zephyr
 ...
 * the pointer.  With the Zephyr network stack there is no lwIP to define the
...
struct netif {
	void *net_dev; /* wlan0 network interface (opaque; used by the blob) */
};
...
/* Interface created/removed: with the Zephyr netdev this already happened at
...
/* IP configuration: with CONFIG_NETWORKING, Zephyr is in charge */
