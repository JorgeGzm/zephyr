...
int macif_tx_start(void *net_if, struct pbuf *buf, void (*cfm_cb)(uint32_t, bool, void *),
		   void *cfm_arg);
...
int wifi_management_ap_start(char *ssid, char *passwd, uint32_t channel, int auth_mode,
			     uint32_t hidden);
...
/* The MAC reports frequencies; the Wi-Fi API wants channel numbers. */

static uint8_t gdwifi_freq_to_chan(uint16_t freq)
{
	if (freq == 2484) {
		return 14;
	}
...
void *net_buf_tx_info(net_buf_tx_t *buf, uint16_t *tot_len, int *seg_cnt, uint32_t seg_addr[],
		      uint16_t seg_len[])
{
	struct gdwifi_txbuf *tx = (struct gdwifi_txbuf *)buf;
	int seg_max = *seg_cnt;
...
int net_if_input(net_buf_rx_t *buf, void *net_if, void *addr, uint16_t len, net_buf_free_fn free_fn)
{
	...
	if (ethertype == 0x888e) {
		wifi_wpa_rx_eapol_event(vif_idx_to_wvif(GDWIFI_VIF_STA), ethertype, frame, len);
		free_fn(buf);
		return 0;
	}
	...
	pkt = net_pkt_rx_alloc_with_buffer(data->iface, len, AF_UNSPEC, 0, K_MSEC(50));
	if (pkt == NULL) {
		free_fn(buf);
		return -1;
	}
...
int net_lpbk_socket_bind(int sock_recv, uint32_t port)
{
	...
	return zsock_bind(sock_recv, (struct sockaddr *)&addr, sizeof(addr)) < 0 ? -1 : 0;
}
...
int net_lpbk_socket_connect(int sock_send, uint32_t port)
{
	...
	return zsock_connect(sock_send, (struct sockaddr *)&addr, sizeof(addr)) < 0 ? -1 : 0;
}
...
int gdwifi_iface_ready(const uint8_t *mac_addr)
{
	...
	if (data->iface != NULL) {
		net_if_set_link_addr(data->iface, data->mac, 6, NET_LINK_ETHERNET);
		net_eth_carrier_off(data->iface);

		/* The link address is valid now, so the interface may start
		 * (see gdwifi_iface_init()).  It stays operationally down
		 * until the association reports the carrier.
		 */

		net_if_flag_clear(data->iface, NET_IF_NO_AUTO_START);

		if (net_if_up(data->iface) < 0) {
			LOG_ERR("failed to bring wlan0 up");
			return -EIO;
		}
	}

	LOG_INF("wlan0 ready (%02x:%02x:%02x:%02x:%02x:%02x)", mac_addr[0], mac_addr[1],
...
static void gdwifi_iface_init(struct net_if *iface)
{
	...
	/* The link address only exists once the vendor firmware has read it
	 * from the efuse and created the VIF, which happens long after the
	 * networking stack comes up.  Keep the interface from auto-starting
	 * until then: ethernet_enable() validates the link address right
	 * after the driver start and rejects the interface without one.
	 * gdwifi_iface_ready() starts it.
	 */
			    ...
			    struct wifi_scan_params *params, scan_result_cb_t cb)
{
	...
	for (uint32_t i = 0; i < results->result_cnt; i++) {
		...
		entry.mac_length = 6;
		entry.rssi = ap->rssi;
		entry.channel = (ap->chan != NULL) ? gdwifi_freq_to_chan(ap->chan->freq) : 0;

		entry.band = WIFI_FREQ_BAND_2_4_GHZ;
		entry.security =
			(ap->akm & CO_BIT(MAC_AKM_PRE_RSN))
				? ((ap->akm & CO_BIT(MAC_AKM_NONE)) ? WIFI_SECURITY_TYPE_NONE
...
static int gdwifi_target_supported(const char *ssid)
{
	...
	uint32_t supported = CO_BIT(MAC_AKM_NONE) | CO_BIT(MAC_AKM_PRE_RSN) | CO_BIT(MAC_AKM_PSK) |
			     CO_BIT(MAC_AKM_PSK_SHA256);
	int ret = 0;
			       ...
			       struct wifi_connect_req_params *params)
{
	...
	if (params->psk_length > 0 && params->psk_length <= WIFI_PSK_MAX_LEN) {
	...
	ret = wifi_management_connect(ssid, psk[0] ? psk : NULL, 1 /* blocked */);
...
static int gdwifi_mgmt_disconnect(const struct device *dev, struct net_if *iface)
{
	int ret;
...
static int gdwifi_mgmt_ap_enable(const struct device *dev, struct net_if *iface,
				 struct wifi_connect_req_params *params)
{
	struct gdwifi_dev_data *data = &g_gdwifi;
	char ssid[WIFI_SSID_MAX_LEN + 1];
	...
	ret = wifi_management_ap_start(ssid, psk[0] ? psk : NULL, channel, auth, 0);
	if (ret == 0) {
...
#if defined(CONFIG_NET_L2_WIFI_MGMT)
	wifi_mgmt_raise_ap_enable_result_event(iface, ret == 0 ? WIFI_STATUS_AP_SUCCESS
							       : WIFI_STATUS_AP_FAIL);
#endif
	return ret == 0 ? 0 : -EIO;
}

static int gdwifi_mgmt_ap_disable(const struct device *dev, struct net_if *iface)
{
...
#if defined(CONFIG_NET_L2_WIFI_MGMT)
	wifi_mgmt_raise_ap_disable_result_event(iface, ret == 0 ? WIFI_STATUS_AP_SUCCESS
								: WIFI_STATUS_AP_FAIL);
#endif
	return ret == 0 ? 0 : -EIO;
}

static int gdwifi_mgmt_iface_status(const struct device *dev, struct net_if *iface,
				    struct wifi_iface_status *status)
{
	...
	status->state = connected ? WIFI_STATE_COMPLETED : WIFI_STATE_DISCONNECTED;
	...
	if (data->mode == WIFI_MODE_AP) {
		/* In softAP mode this interface *is* the BSS. */

		memcpy(status->bssid, data->mac, sizeof(status->bssid));
	} else if (connected) {
		struct mac_vif_status vif;

		/* Ask the MAC about the peer.  Reporting our own address as
		 * the BSSID, as this driver first did, is simply wrong, and
		 * the scan cache is no help: a connect does not require a
		 * preceding scan.
		 */

		if (macif_vif_status_get(GDWIFI_VIF_STA, &vif) == 0) {
			memcpy(status->bssid, vif.sta.bssid.array, sizeof(status->bssid));
			status->channel = gdwifi_freq_to_chan(vif.chan.prim20_freq);
			status->rssi = vif.sta.rssi;
		}
	}
...
static void gdwifi_bringup(void *a, void *b, void *c)
{
...
#if defined(CONFIG_WIFI_GDWIFI_BLE_VENDOR)
	{
	...
	wifi_management_connect(CONFIG_WIFI_GDWIFI_TEST_SSID, CONFIG_WIFI_GDWIFI_TEST_PSK, 1);
#endif
}

K_THREAD_DEFINE(gdwifi_boot, 4096, gdwifi_bringup, NULL, NULL, NULL, K_PRIO_PREEMPT(8), 0, 500);
...
NET_DEVICE_DT_INST_DEFINE(0, gdwifi_dev_init, NULL, &g_gdwifi, NULL, CONFIG_WIFI_INIT_PRIORITY,
			  &gdwifi_api, ETHERNET_L2, NET_L2_GET_CTX_TYPE(ETHERNET_L2), NET_ETH_MTU);
