 ...
 *
 * Polling controller driver. The transfer sequence mirrors the bare-metal
...
static uint32_t i2c_gd32_v2_timing(uint32_t pclk, uint32_t bitrate)
{
	...
	return ((presc - 1U) << 28) | (scldel << 20) | (sdadel << 16) | (sclh << 8) | scll;
}
...
static int i2c_gd32_v2_msg(uint32_t reg, uint16_t addr, struct i2c_msg *msg, bool do_stop)
{
	...
	ctl1 = I2C_CTL1(reg);
	ctl1 &= ~(I2C_CTL1_SADDRESS | I2C_CTL1_TRDIR | I2C_CTL1_BYTENUM | I2C_CTL1_RELOAD |
		  I2C_CTL1_AUTOEND | I2C_CTL1_START | I2C_CTL1_STOP);
	ctl1 |= ((uint32_t)addr << 1) & I2C_CTL1_SADDRESS;
	if (is_read) {
		ctl1 |= I2C_CTL1_TRDIR; /* controller receive */
	}
...
static int i2c_gd32_v2_transfer(const struct device *dev, struct i2c_msg *msgs, uint8_t num_msgs,
				uint16_t addr)
{
	...
	for (uint8_t i = 0U; i < num_msgs; i++) {
		bool do_stop = ((i == (num_msgs - 1U)) || (msgs[i].flags & I2C_MSG_STOP) != 0U);
...
static int i2c_gd32_v2_configure(const struct device *dev, uint32_t dev_config)
{
	...
	if ((dev_config & I2C_MODE_CONTROLLER) == 0U) {
		return -ENOTSUP; /* controller mode only */
	}
...
#define I2C_GD32_V2_INIT(inst)                                                                     \
	...
	I2C_DEVICE_DT_INST_DEFINE(inst, i2c_gd32_v2_init, NULL, &i2c_gd32_v2_data_##inst,          \
				  &i2c_gd32_v2_cfg_##inst, POST_KERNEL, CONFIG_I2C_INIT_PRIORITY,  \
